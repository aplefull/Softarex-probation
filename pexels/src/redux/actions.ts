import {HIDE_SEARCH_BAR, LOAD_HEADER_IMAGE, LOAD_PHOTOS, SHOW_SEARCH_BAR} from "./types";

export function getHeaderImage() {
    return async (dispatch: Function) => {
        const response = await fetch(`https://api.pexels.com/v1/search?query=mountains&orientation=landscape&size=medium&per_page=20&page=1`, {
            headers: {
...
export function loadPhotos() {
    return async (dispatch: Function) => { //https://api.pexels.com/v1/search?query=nature&size=small&per_page=20&page=1
