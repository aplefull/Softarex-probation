...
function FilterButton({ caption }: FilterButtonProps) {
  const dispatch = useDispatch();
  const { orientation, size, color } = useSelector((state: RootState) => state.filters);

  const handleOrientationClick = useCallback((e: MouseEvent<HTMLLIElement>) => {
    dispatch(changeOrientationFilter(e.currentTarget.dataset.value || ''));
  }, [dispatch]);

  const handleSizeClick = useCallback((e: MouseEvent<HTMLLIElement>) => {
    dispatch(changeSizeFilter(e.currentTarget.dataset.value || ''));
  }, [dispatch]);

  const handleColorClick = useCallback((e: MouseEvent<HTMLDivElement | HTMLLIElement>) => {
  ...
  const handleColorInputChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
  ...
  const renderIcon = useCallback((str: string) => {
    switch (str) {
      case 'Orientation':
  ...
  const renderDropdown = useCallback(
            ...
            <div className={`${styles.dropdownWrapper} ${styles.size}`}>
              <ul>
                ...
                <li
                  ...
                  className={size === 'medium' ? styles.selected : ''}
                >
                  ...
                  <div className={styles.colorOptions} onClick={handleColorClick}>
                    ...
                    <div
                      className={`${styles.colorOption} ${styles.brown} ${
                        color === 'brown' ? styles.selectedColor : ''
                      }`}
                      data-value={'brown'}
                    >
                      <CheckmarkIcon />
                    ...
                    <div
                      className={`${styles.colorOption} ${styles.white} ${
                        color === 'white' ? styles.selectedColor : ''
                      }`}
                      data-value={'white'}
                    >
                      <CheckmarkIcon />
                    ...
                    <div
                      className={`${styles.colorOption} ${styles.black} ${
                        color === 'black' ? styles.selectedColor : ''
                      }`}
                      data-value={'black'}
                    >
                      <CheckmarkIcon />
    ...
    [color, orientation, size, handleColorClick, handleColorInputChange, handleSizeClick, handleOrientationClick]
  );
